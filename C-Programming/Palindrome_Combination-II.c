...
// Function to check if a subsequnce is already generated
int seen(char **subsets, int subsetCount, char *curr_combination) {
    for (int i = 0; i < subsetCount; i++) {
        if (areStringsEqual(subsets[i], curr_combination)) {
            return 1; // Subseq is already generated
        }
    }
    return 0; // Subseq is not generated yet
}
...
void genCombinations(char *arr, int arrSize, int k, char *curr_combination, int index, char **subsets, int *subsetCount) {
    if (index == k) {
        ...
        if (!seen(subsets, *subsetCount, curr_combination) && isPalindrome(curr_combination)) {
...
void stringSubsets(char *arr) {
    ...
    for (int k = 0; k < arrSize + 1; k++) {
        ...
        curr_combination[k] = '\0'; // Null-terminate the string in last index;
