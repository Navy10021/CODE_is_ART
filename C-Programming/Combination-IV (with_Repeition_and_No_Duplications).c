...
void generateCombinations(char set[], int n, int k, char current[], int index, char**arr, int*arr_cnt) {
    if (index == k) {
    ...
    for (int i = 0; i < n; i++) {
        current[index] = set[i];
        generateCombinations(set, n, k, current, index + 1, arr, arr_cnt);
    }
}
...
void combinations(char set[], int n, int k){
    ...
    generateCombinations(set, n, k, current, 0, arr, &arr_cnt);
