...
class Point {
    ...
    public:
        Point(double _x, double _y) : x(_x), y(_y), isValid(true) {
            //isValid = (0 <= _x && _x <= 100) && (0 <= _y && _y <= 100);
            setX(_x);
            setY(_y);
        }
        //If using "Accessor"
...
int main() {
    ...
    // Invalid Point(x, y)
    shapes.push_back(new Line(-1,-1, 101,101));
    shapes.push_back(new Square(0, 0, 200));
