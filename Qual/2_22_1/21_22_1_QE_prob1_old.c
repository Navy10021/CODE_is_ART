...
int substring(char* s, char* t){ // t in s?
    return strstr(s, t) != NULL; // t가 s에 포함되는지 ? t in s? 포함이 되면 !NULL(NULL이 아니고) 그 자리를 가리키는 포인터 리턴, 포함이 안되면 NULL;
}
    ...
    *returnSize = 0;
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j <= n; j++) {
            ...
            strncpy(substr, &s[i], j - i); // (j-i)개만큼 copy = python slicing ; substr = s[i:j]
            substr[j - i] = '\0'; // substr의 마지막 위치를 '\0'으로 바꿔줌
            if (palindrome(substr)) {
                int found = 0;
                for (int k = 0; k < *returnSize; k++) {
                    if (strcmp(substr, result[k]) == 0) { // strcmp : 문자열 비교 0이면 같음
                        found = 1;
                        break;
                    }
                }
